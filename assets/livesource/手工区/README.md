...
```python
# 注意：手工区文件直接读取并添加到频道列表，不经过process_channel_line函数
zhejiang_lines += zhejiang_manual  # 直接追加，不检查黑名单
guangdong_lines += guangdong_manual  # 直接追加，不检查黑名单
# ... 其他地区同理

# 专享源文件也是直接读取
read_txt_to_array('assets/livesource/手工区/优质央视.txt')  # 不检查黑名单
read_txt_to_array('assets/livesource/手工区/优质卫视.txt')  # 不检查黑名单
```

这意味着：

· 手工区源不受黑名单限制
· 即使某个URL在黑名单中，手工区仍然会包含它
· 这是设计上的故意行为，因为手工区源被视为"可信源"
...
```python
# 地区源的处理：直接追加，可能产生重复
zhejiang_lines += zhejiang_manual  # 可能和自动采集的zhejiang_lines重复

# 但在最终输出时会进行去重：
["🧵浙江频道,#genre#"] + sort_data(zhejiang_dictionary, correct_name_data(corrections_name, zhejiang_lines)) + ['\n']
# 注意：这里没有使用set()去重，保留了所有源

# 对比其他分类有明确去重：
["🐱动·画·片,#genre#"] + sort_data(cartoon_dictionary, set(correct_name_data(corrections_name,cartoon_lines)))  # 使用set去重
```

去重情况分析：

分类 去重方式 手工区影响
地区频道 不去重 手工区源会追加，可能导致重复
动画片、纪录片等 set()去重 重复的手工区源会被过滤
专享分类 直接读取 独立分类，不与自动源混合
...
for whitelist_line in whitelist_auto_lines:
    if "#genre#" not in whitelist_line and "," in whitelist_line and "://" in whitelist_line:
        ...
        try:
            ...
            response_time = 60000
        if response_time < 2000:  # 响应时间小于2秒的才使用
            process_channel_line(",".join(whitelist_parts[1:]))
```

手工区质量控制机制：

1. 人工筛选：手工区源由人工维护，质量相对有保障
2. 白名单参考：部分源可能来自白名单筛选结果
3. 响应时间筛选：response_time < 2000 确保源响应快速
...
aktv_text = get_http_response(aktv_url)  # 优先从GitHub获取

if aktv_text:
    aktv_text = convert_m3u_to_txt(aktv_text)
    aktv_lines = aktv_text.strip().split('\n')
else:
    aktv_lines = read_txt_to_array('assets/livesource/手工区/AKTV.txt')  # 备用本地文件

# AKTV源会经过process_channel_line处理
for line in aktv_lines:
    process_channel_line(line)  # 这意味着会检查黑名单和去重
```
...
```python
# 路径1：普通URL源（经过完整处理）
process_url(url) → process_channel_line(line) → [黑名单检查] → [URL去重] → [名称修正] → 分类存储

# 路径2：手工区地区源（绕过大部分检查）
直接读取 → 直接追加到对应列表 → 输出时只进行名称修正

# 路径3：手工区专享源（独立分类）
直接读取 → 独立分类输出 → 不混合，不去重

# 路径4：AKTV源（混合处理）
外部获取或本地读取 → process_channel_line → [黑名单检查] → [URL去重] → 分类存储
```
...
1. 手工区源 vs 自动采集源
   ```python
   # 假设手工区浙江频道.txt包含：浙江卫视,http://example.com/zjws
   # 自动采集也可能包含相同的URL
   # 结果：在浙江频道分类中出现重复
   ```
2. 手工区专享源 vs 普通分类源
   ```python
   # 手工区优质央视.txt和自动采集的央视可能有重叠
   # 但专享源是独立分类，所以用户会看到两个分类都有相同频道
   ```
3. 今日推荐 vs 其他源
   ```python
   # get_random_url可能选择已在其他分类中的源
   # 结果：同一个URL出现在"今日推荐"和原始分类中
   ```
...
def process_manual_source(file_path):
    ...
    for line in lines:
        if ',' in line and '://' in line:
            _, url = line.split(',', 1)
            if url not in combined_blacklist:  # 检查黑名单
...
def add_manual_sources_with_deduplication(existing_lines, manual_lines):
    ...
    for line in manual_lines:
        if ',' in line:
            _, url = line.split(',', 1)
            if url not in existing_urls:  # 去重
                existing_lines.append(line)
                existing_urls.add(url)

# 建议3：增加手工区质量标签
["🧵浙江频道(手工精选),#genre#"] + manual_zhejiang_lines + ['\n']
["🧵浙江频道(自动采集),#genre#"] + auto_zhejiang_lines + ['\n']
```

9. 当前实际影响

1. 黑名单影响：手工区源不受黑名单限制
2. 去重影响：手工区地区源可能产生重复，其他分类会去重
3. 质量保证：手工区源被认为质量更高，所以特殊对待
4. 用户体验：用户可能会看到重复频道，但有更多备选源

总结：手工区设计为"特权区域"，牺牲了一定的去重一致性，以提供更稳定、更多样的直播源选择。这种设计体现了"宁可重复，不可缺失"的备份思想。
...
本文件定义了直播源处理工具中的所有标准格式，便于查阅和维护。
================================================================================

一、频道分类处理分发格式 (process_channel_line函数中使用)
--------------------------------------------------------------------------------

格式1: 央视频道格式 (特殊关键字匹配)
-----------------------------------
用途: 识别所有包含"CCTV"的频道
特点: 不依赖字典，直接通过关键词"CCTV"判断
示例:
...
格式3: 关键词模糊匹配格式
-----------------------------------
用途: 体育赛事、咪咕赛事等需要部分匹配关键词的分类
特点: 只要频道名称包含字典中的任意关键词即可
示例:
...
二、播放列表数据处理组合格式 (播放列表生成中使用)
--------------------------------------------------------------------------------
格式说明: ["分组标题,#genre#"] + 数据处理函数 + ['\n'] + \

格式1: 字典排序 + 名称修正 (未去重)
-----------------------------------
用途: 需要按字典排序且名称修正，但允许重复项的频道
示例:
    ["🌐央视频道,#genre#"] + sort_data(yangshi_dictionary, 
            correct_name_data(corrections_name, yangshi_lines)) + ['\n'] + \

格式2: 字典排序 + 去重 + 名称修正
-----------------------------------
用途: 需要按字典排序、去重且名称修正的频道
示例:
    ["🐯广东频道,#genre#"] + sort_data(guangdong_dictionary,
            set(correct_name_data(corrections_name, guangdong_lines))) + ['\n'] + \

格式3: 去重 + 名称修正 + 字符排序
-----------------------------------
用途: 没有字典文件，按字母排序的去重频道
示例:
    ["🐘河南频道,#genre#"] + sorted(set(correct_name_data(corrections_name, henan_lines))) + ['\n'] + \

格式4: 去重 + 字符排序 (无名称修正)
-----------------------------------
用途: 名称规范，只需去重和按字母排序的频道
示例:
    ["🎵音乐频道,#genre#"] + sorted(set(music_lines)) + ['\n'] + \

格式5: 字典排序 + 去重 (无名称修正)
-----------------------------------
用途: 名称规范，只需按字典排序和去重的频道
示例:
    ["📻收·音·机,#genre#"] + sort_data(radio_dictionary, set(radio_lines)) + ['\n'] + \

格式6: 字典排序 + 名称修正 (未去重)
-----------------------------------
用途: 需要按字典排序和名称修正的传统频道
示例:
    ["📺电·视·剧,#genre#"] + sort_data(tv_drama_dictionary,
...
格式9: 字典排序 + 去重 + 名称修正 (紧凑格式)
-----------------------------------
用途: 专业频道的标准处理方式
示例:
    ["🎥纪·录·片,#genre#"] + sort_data(documentary_dictionary,
...
示例:
    ["🕒更新时间,#genre#"] + [version] + [about] + [MTV1] + [MTV2] + [MTV3] + [MTV4] + [MTV5] + 
            read_txt_to_array('assets/livesource/手工区/about.txt') + ['\n']

格式13: 字典排序 + 去重 + 名称修正 (动画片专用)
-----------------------------------
用途: 动画片频道的标准处理
示例:
    ["🐱动·画·片,#genre#"] + sort_data(cartoon_dictionary,
...
用途: 戏曲频道的传统处理方式
示例:
    ["🎭戏曲频道,#genre#"] + sort_data(traditional_opera_dictionary,
...
用途: 春晚节目的标准处理
示例:
    ["🧨历届春晚,#genre#"] + sort_data(spring_festival_gala_dictionary,
            set(spring_festival_gala_lines)) + ['\n'] + \

格式16: 去重 + 字符排序 + 名称修正 (景区直播)
-----------------------------------
用途: 景区直播的简化处理
示例:
...
================================================================================
推荐标准格式: 字典排序 + 去重 + 名称修正 (格式2/9/13)
统一建议使用: sort_data(字典名, set(correct_name_data(corrections_name, 数据列表)))
================================================================================
"""