...
class GlobalState:
    def __init__(self):
        ...
        self.other_lines = []  # 其他未分类频道行
        self.other_lines_url = set()  # 其他未分类频道URL（用于去重）
...
def load_dictionaries():
    ...
    if "weishi" in dictionaries:
        ...
        if weishi_list:
            dictionaries["weishi"] = set(weishi_list)
            print(f"   ⚡ 卫视频道转换为集合: {len(weishi_list)}条")
    ...
    for cat_id in high_frequency_categories:
        if cat_id in dictionaries:
            dict_list = dictionaries[cat_id]
            if dict_list and len(dict_list) > 20:
                dictionaries[cat_id] = set(dict_list)
                print(f"   ⚡ {CHANNEL_CONFIG[cat_id]['title']}转换为集合: {len(dict_list)}条")
    print(f"✅ 数据结构优化完成")
...
def process_channel_line(line, dictionaries, is_manual=False):
    ...
    if "#genre#" not in line and "#EXTINF:" not in line and "," in line and "://" in line:
        ...
        # 2. 卫视频道（使用集合O(1)查找）
        if not matched:
            weishi_dict = dictionaries.get("weishi")
            if weishi_dict and channel_name in weishi_dict:
                CHANNEL_CONFIG["weishi"]["lines"].append(process_name_string(processed_line))
                matched = True
        
        # 3. 体育赛事（关键词匹配）
        if not matched:
            tyss_keywords = dictionaries.get("tyss")
            if tyss_keywords:
                for keyword in tyss_keywords:
                    if keyword and keyword[0] in channel_name and keyword in channel_name:
        ...
        # 4. 咪咕赛事（关键词匹配）
        if not matched:
            mgss_keywords = dictionaries.get("mgss")
            if mgss_keywords:
                for keyword in mgss_keywords:
                    if keyword and keyword[0] in channel_name and keyword in channel_name:
        ...
        # 5. 定制分类（精确匹配）
        if not matched:
            # 预定义需要精确匹配的分类（排除已处理的特殊分类）
            exact_match_categories = [
                cat_id for cat_id in CATEGORY_ORDER 
                if cat_id not in ["yangshi", "weishi", "tyss", "mgss"]
            ]
            
            for cat_id in exact_match_categories:
                dict_data = dictionaries.get(cat_id)
                if not dict_data:
                    continue
                
                if isinstance(dict_data, set):
                    # 集合查找 O(1)
                    if channel_name in dict_data:
                        CHANNEL_CONFIG[cat_id]["lines"].append(processed_line)
                        matched = True
                        break
                else:
                    # 列表查找 O(n)
